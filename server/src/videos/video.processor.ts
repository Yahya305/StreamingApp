...
export class VideoProcessor extends WorkerHost {
    ...
    async process(job: Job<any, any, string>): Promise<any> {
        ...
        try {
            ...
            let localInputPath = filePath;

            // If the input is an R2 URL, download it first
            if (job.data.isUrl) {
                this.logger.log(
                    `Downloading original video from R2: ${filePath}`
                );
                const downloadPath = path.join(
                    os.tmpdir(),
                    `original-${videoId}${path.extname(new URL(filePath).pathname)}`
                );

                // Simple download using fetch (available in Node 18+) or axios
            ...
            this.logger.log(
                `Uploading ${processedFiles.length} files to R2...`
            );
            ...
            if (localInputPath && fs.existsSync(localInputPath)) {
                fs.unlinkSync(localInputPath); // Original local/downloaded file
            }
        } catch (error) {
            this.logger.error(
